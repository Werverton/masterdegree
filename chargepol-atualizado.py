...
#direct = 'C:\\Users\\werve\\Downloads\\RELAMP_LMA_20190417_level2'

#direct = 'C:\\Users\\werve\\Downloads\\RELAMP_LMA_20190419_level3'
...
def write_output(filename_output, pos_time, pos_zmin, pos_zwid, pos_flax, pos_flay, pos_fla_lon, pos_fla_lat, neg_time, neg_zmin, neg_zwid, neg_flax, neg_flay, neg_fla_lon, neg_fla_lat):
    ...
    pos_time = np.array(pos_time, dtype=float).flatten()
    pos_zmin = np.array(pos_zmin, dtype=float).flatten()
    pos_zwid = np.array(pos_zwid, dtype=float).flatten()
    pos_flax = np.array(pos_flax, dtype=float).flatten()
    pos_flay = np.array(pos_flay, dtype=float).flatten()
    pos_fla_lon = np.array(pos_fla_lon, dtype=float).flatten()
    pos_fla_lat = np.array(pos_fla_lat, dtype=float).flatten()
    neg_time = np.array(neg_time, dtype=float).flatten()
    neg_zmin = np.array(neg_zmin, dtype=float).flatten()
    neg_zwid = np.array(neg_zwid, dtype=float).flatten()
    neg_flax = np.array(neg_flax, dtype=float).flatten()
    neg_flay = np.array(neg_flay, dtype=float).flatten()
    neg_fla_lon = np.array(neg_fla_lon, dtype=float).flatten()
    neg_fla_lat = np.array(neg_fla_lat, dtype=float).flatten()
...
if os.path.isfile(direct):
    ...
    filenames = glob.glob(os.path.join(direct, '*.nc'))
else:
    filenames = glob.glob(direct)
    ...
    for j in range(0,flid.size):
        ...
        if np.abs(pb_vert_speed) > min_ver_speed and mse < max_mse and len(whch[0]) > 10: # Ensure enough points for clustering
            ...
            # Downward PB:
                ...
                # Negative layer is the upper cluster, positive is the lower cluster:
                pos_sour = z_lower
                neg_sour = z_upper
                pos_t = t_lower
                neg_t = t_upper
